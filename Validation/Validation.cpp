...
public:
	int validteStudent(Student student)
	{
		if (student.getName().size() == 0 ||
			student.getName().size() > 9 ||
			student.getName().size() < 2)
		{
...
public:
	int validteCourse(Course course)
	{
		if (course.getName().size() == 0 ||
			course.getName().size() < 3)
		{
			cout << "Invalid Name !" << endl;
		}
		else if (course.getHour() > 24 || course.getHour() < 4)
		{
...
public:
	int validteTeacher(Teacher teacher)
	{
		if (teacher.getName().size() == 0 ||
			teacher.getName().size() > 9 ||
			teacher.getName().size() < 3)
		{
			cout << "Invalid Name !" << endl;
		}
		else if (teacher.getAge() > 60 || // 25 60
				 teacher.getAge() < 25)
		{
			cout << "Invalid Age !" << endl;
		}
